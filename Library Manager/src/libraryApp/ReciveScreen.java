...
public class ReciveScreen extends JFrame implements ActionListener {
	...
	public ReciveScreen() {
		...
		JLabel lblLend = new JLabel("RECIVE BOOK");
		...
		if(command.equals("Close")) {
			...
			if(comboBook.getSelectedIndex() !=0 ) {
				...
				if(comboMember.getSelectedIndex() != 0) {
					...
					App.books.get(book).bookReturn(member);;
					comboMember.setEnabled(false);
	...
	private void updateMembers(Books book) {
		DefaultComboBoxModel<String> model = new DefaultComboBoxModel<String>();
		model.addElement("Books");
		Member[] arr = book.getBooksGiven();
		for(Member m: arr) {
